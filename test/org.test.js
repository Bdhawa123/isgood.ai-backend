...
beforeEach(async () => {
    testOrgObject = await supertest(app).post("/api/org/create").set(Auth).send(org[2]);
});
...
describe('Organisations', () => {
    ...
    it("Updates an Organisation", async () => { 
        ...
        const response = await supertest(app).patch(`/api/org/${orgId}`).set(Auth).send(updateOrgData)
        expect(response.statusCode).toBe(200)
        const org_data = await db.select("*").from("org").where("id", orgId).first();
    ...
    it("Updates selected fields of an Organisation", async () => {
        const orgId = testOrgObject.body.id;
        const response = await supertest(app).patch(`/api/org/${orgId}`).set(Auth).send({name: "Apple", description: ""});
        expect(response.statusCode).toBe(200);
