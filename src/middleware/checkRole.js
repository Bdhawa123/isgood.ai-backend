const {RoleService, getOrgRole, getProjectRole} = require('../services/role-service')
...
function checkProjectUpdate(req, res, next) {
    ...
    getOrgRole(
        ...
        .then(roles => {
            ...
            if(orgRole !== 'ORGANIZATION_OWNER' && orgRole !== 'ORGANIZATION_MANAGER') {
                return res.status(403).json({
                    error: {message: 'Unauthorized Request'}
                })
            }
            getProjectRole(
                ...
                .then(role => {
                    ...
                    if(projectRole !== 'PROJECT_OWNER' && projectRole !== 'PROJECT_MANAGER') {
                        return res.status(403).json({
                            error: {message: 'Unauthorized Request'}
                        })
                    }
